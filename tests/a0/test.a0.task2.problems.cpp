...
Test test_a0_task2_problems_numerical("a0.task2.problems.numerical", []() {
    ...
    int factor = 3;
    auto helper = [&](int x, int y) { return ((float)(x) < (float)((float)(y)/(float)(factor))); };

    int j = 0;
    for (auto& v : target) { //goes through target 3 times
        
        for (auto& i : v) { //goes through target 3 more times
...
Test test_a0_task2_problems_vector("a0.task2.problems.vector", []() {
    ...
    for (std::vector<int>::iterator it = one_to_ten.begin() ; it != one_to_ten.end(); ++it) last_element = *it;
    ...
    for (size_t i = 0; i < vec1.size(); i++) {
        for (size_t j = 0; j < vec2.size(); j++) {
            for (size_t k = 0; k < vec3.size(); k++) {
                ...
                if ((vec1.at(i) == vec2.at(j)) && (vec1.at(i) == vec3.at(k)) && (vec2.at(j) == vec3.at(k))) count++;
            }
        }
    }
